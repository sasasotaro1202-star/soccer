...
# 2010-11 through 2025-26: full requested chronological range.
...
def mom_score(p, winp, elo, opp):
    ...
    production=np.clip(.34*p.goals/max(1,p.matches)+.26*p.assists/max(1,p.matches)+.24*p.xg/max(1,p.matches)+.10*p.key_passes/max(1,p.matches),0,1.5)
...
def main():
    ...
    # Build canonical group sizes row-by-row.  The previous groupby-based map could
    # miss a group when a source supplied SeasonStart with a different numeric dtype
    # (e.g. 2010 vs 2010.0) or a non-standard value.  Canonicalizing here guarantees
    # the key used during processing is identical to the key used for counting.
    group_sizes=defaultdict(int)
    for _r in matches[["League","SeasonStart","Season"]].itertuples(index=False):
        try:
            _yy=int(float(_r.SeasonStart))
        except Exception:
            _yy=str(_r.SeasonStart).strip()
        _ss=str(_r.Season).strip()
        group_sizes[f"{str(_r.League).strip()}|{_yy}|{_ss}"] += 1
    ...
    if ck:
        ...
        global_elo=dict(ck.get("global_elo",{})); cursor=int(ck.get("cursor",0))
        ...
        blends={}; leagues={c:League() for c in LEAGUES}; players=defaultdict(Player); last_dates={}; h2h={}
        global_elo={}; cursor=0; active_seasons={}
    for c in LEAGUES:
    ...
    while cursor < len(matches):
        ...
        lw,mw,sw=blends.get(code,(.56,.28,.16))
        ...
        ge_h=global_elo.get(row.HomeTeam,1500.0); ge_a=global_elo.get(row.AwayTeam,1500.0)
        ...
        expected_group_size=group_sizes.get(gkey)
        if expected_group_size is None:
            # Defensive fallback: never crash the long-running backtest because a
            # source produced an unexpected grouping key.  Recount the exact group.
            mask=(matches["League"].astype(str).str.strip()==code)
            try:
                mask &= pd.to_numeric(matches["SeasonStart"],errors="coerce").eq(year)
            except Exception:
                pass
            mask &= matches["Season"].astype(str).str.strip().eq(season)
            expected_group_size=int(mask.sum())
            group_sizes[gkey]=expected_group_size
        if expected_group_size > 0 and group_seen[gkey]>=expected_group_size:
            done.add(gkey)
            if len(hist)>=360: blends[code]=optimize_blend(hist)
            if bundle:
