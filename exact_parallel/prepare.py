...
SNAP=ROOT/"exact_parallel"/"baseline"; SNAP.mkdir(parents=True,exist_ok=True)
...
# Determine the input cardinality from the same canonical loader used by V9.
...
targets=[int(total*0.25),int(total*0.50),int(total*0.75)]
seen=set(); original=backtest.save_state
...
v12_runner.main()
# A complete baseline must have one V9 result per processed canonical match.
import pandas as pd
v9=pd.read_csv(ROOT/"backtest_results_v9.csv",low_memory=False)
v12=pd.read_csv(ROOT/"backtest_results_v12.csv",low_memory=False)
...
for n in range(1,4):
    ck=backtest.load_state.__globals__.get('CHECKPOINT')
    # boundary files already copied; their cursors are read for the manifest below.
rows=[]
for n in range(1,4):
    import gzip,pickle
    with gzip.open(SNAP/f"checkpoint_boundary_{n}.pkl.gz","rb") as f: ck=pickle.load(f)
    rows.append({"boundary":n,"cursor":int(ck["cursor"])})
